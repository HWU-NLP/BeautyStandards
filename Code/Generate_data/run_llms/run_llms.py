    ...
    for i, s in enumerate(generation_output.sequences):
        ...
        if args.model_name == "llama3.1_instruct" or args.model_name == "deepseek_llm":  
            ...
            if args.model_name == "deepseek_llm":  
                # DeepSeek format typically just generates responses
                prompt = prompt_dataset.prompts[current_idx + i]
                if prompt in full_output:
                    parts = full_output.split(prompt)
                    if len(parts) > 1:  
...
MODEL_NAMES = {
    ...
    "deepseek_llm": "deepseek-ai/deepseek-llm-7b-chat"
}
...
instruction_chat = """You are writing a character for a book. Create a detailed physical description of a person based on the input.
    ...
    for batch in dataloader:
        ...
        for i in range(batch_size):
            if sample_idx < len(prompt_dataset):  # so that the batches don't go over the len of prompts
                ...
                # this part to save the output without newlines
                import re
